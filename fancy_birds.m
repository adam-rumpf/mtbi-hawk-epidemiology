function results = fancy_birds(evf,tf,H0,S0,I0,R0,sigma,flag)
% results = fancy_birds(evf,tf,H0,S0,I0,R0,sigma,flag)
%
% Last revised 7/26/11, 1:09pm
%
% Runs a Markov process on a hawk population that consumes infected dove meat.
% Lets you select which model variant to use.
%
% Inputs:
...
% sigma ~ infection rate in food (use a negative value to default to ~42%)
...
% Parameters:
% muH = 1 / (365*7); % natural adult deaths
  muH = 1 / (365*5);
% muH = 1 / (365*4);
...
% b = (3.44 / 365 / 2) * 0.75 * 0.835; % average hatchlings per hawk
...
if (flag)
	...
	% title(['Markov process for hawk population, continuous time, sigma = ' num2str(sigma)]);
end