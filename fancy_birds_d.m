function results = fancy_birds_d(evf,tf,H0,S0,I0,R0,d,flag)
% results = fancy_birds_d(evf,tf,H0,S0,I0,R0,d,flag)
%
...
% d ~ disease death rate (use a negative value to default to ~0.009)
...
if (d < 0)
	% d = 0.4 / (365*2);
	%  d = 0.41 / 45; % disease death rate based on our research
     d = 0.41 / 40;
end
...
if (flag)
	...
	xlabel('Day');
	ylabel('Hawks');
	title(['Markov process for hawk population, continuous time, d = ' num2str(d)]);
end